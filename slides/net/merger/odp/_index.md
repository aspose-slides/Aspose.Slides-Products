...
+  Load all the ODP files with full path.
+  Make one document as the base file
+  Call the relevant method for concatenating and merging files one by one.
+  Call the Save() method and pass the file name (full path) and format (ODP) as a parameter.
+  Now you can open and use the ODP file in Microsoft Office, Adobe PDF or any other compatible program.
