...
{{< blocks/products/pf/agp/faq-item question="What is the difference between the online signing app and the API?" answer="The free app adds a drawn, typed or image signature — a visible mark on a slide. The API applies a digital signature to a PPTX, which is a cryptographic assertion about the file, not a picture." >}}
{{< blocks/products/pf/agp/faq-item question="Which format can be digitally signed?" answer="PPTX. Digital signatures are part of the OOXML package format, so the legacy binary formats cannot carry one." >}}
